...
from Rudra.database.pmpermitdb import get_approved_users, pm_guard
import Rudra.database.pmpermitdb as Rudra
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Rudra.set_block_message(Rudra.BLOCKED)
        await message.edit("**Block message set to default**.")
        return
    await Rudra.set_block_message(f"`{arg}`")
...
async def allow(client, message):
    ...
    pmpermit, pm_message, limit, block_message = await Rudra.get_pm_settings()
    await Rudra.allow_user(chat_id)
