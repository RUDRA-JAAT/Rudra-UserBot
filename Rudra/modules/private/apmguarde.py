...
from Rudra.modules.help import *
...
async def pmguard(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "off":
        await Rudra.set_pm(False)
        await message.edit("**PM Guard Deactivated**")
    if arg == "on":
        await Rudra.set_pm(True)
...
async def setpmmsg(client, message):
    ...
    if not arg:
        ...
        return
    if arg == "default":
        await Rudra.set_permit_message(Rudra.PMPERMIT_MESSAGE)
        await message.edit("**Anti_PM message set to default**.")
        return
    await Rudra.set_permit_message(f"`{arg}`")
